...
void pasarTestigo(Semaphore &testigo, const int silla, const bool hayFila,
                  Semaphore *rprimero[], Semaphore &rsegundo,
                  Semaphore &rtercero, Semaphore &rcuarto, Semaphore &rquinto,
                  Semaphore &rsexto, const int pareja[], int &terminado,
                  const bool examen_fin[], int a[], int &b, int &c, int &d,
                  int &e, int &f, int &levantado) {
  bool ifComple = false;
...
//----------------------------------------------------
void Estudiante(const int nip, int &fila, bool &hayFila, int D[N_FIL][N_COL],
                int &silla, int resultado[], bool examen_fin[], int &silla1,
                int &silla2, int pareja[], Semaphore *rprimero[],
                Semaphore &rsegundo, Semaphore &rtercero, Semaphore &rcuarto,
                Semaphore &rquinto, Semaphore &rsexto, Semaphore &testigo,
                int a[], int &b, int &c, int &d, int &e, int &f, int &terminado,
                int &levantado) {
  ...
  pasarTestigo(testigo, silla, hayFila, rprimero, rsegundo, rtercero, rcuarto,
               rquinto, rsexto, pareja, terminado, examen_fin, a, b, c, d, e, f,
               levantado); //>
  ...
  if (nip < miPareja) {
    ...
    // mostrar resultados
    pasarTestigo(testigo, silla, hayFila, rprimero, rsegundo, rtercero, rcuarto,
                 rquinto, rsexto, pareja, terminado, examen_fin, a, b, c, d, e,
                 f, levantado); //> // cambiar couts
...
//----------------------------------------------------
void Profesor(int &silla, int &silla1, int &silla2, int pareja[], int &fila,
              bool &hayFila, Semaphore *rprimero[], Semaphore &rsegundo,
              Semaphore &rtercero, Semaphore &rcuarto, Semaphore &rquinto,
              Semaphore &rsexto, bool examen_fin[], int &terminado,
              Semaphore &testigo, int a[], int &b, int &c, int &d, int &e,
              int &f, int &levantado) {
  for (int i = 0; i < N_FIL; i++) {
...
int main() {
  ...
  Profe = thread(&Profesor, ref(silla), ref(silla1), ref(silla2), ref(pareja),
                 ref(fila), ref(hayFila), ref(rprimer), ref(rsegundo),
                 ref(rtercero), ref(rcuarto), ref(rquinto), ref(rsexto),
                 ref(examen_fin), ref(terminado), ref(testigo), ref(a), ref(b),
                 ref(c), ref(d), ref(e), ref(f), ref(levantado));

  for (int i = 0; i < N_EST; i++) {
    Estu[i] = thread(&Estudiante, i, ref(fila), ref(hayFila), ref(D),
                     ref(silla), ref(resultado), ref(examen_fin), ref(silla1),
                     ref(silla2), ref(pareja), ref(rprimer), ref(rsegundo),
                     ref(rtercero), ref(rcuarto), ref(rquinto), ref(rsexto),
                     ref(testigo), ref(a), ref(b), ref(c), ref(d), ref(e),
                     ref(f), ref(terminado), ref(levantado));
  }
